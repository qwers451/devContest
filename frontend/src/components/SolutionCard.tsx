          ...
          <div className="flex-1 px-5 py-4 min-w-0">
            <div className="flex items-center gap-2 mb-2.5 flex-wrap">
              ...
              {solution.avg_score != null && (
                <span
                  className="px-2 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300"
              ...
              {solution.critical_issues && (
                <span
                  className="px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300"
                  title="ИИ выявил критические нарушения ТЗ"
                >
            ...
            {solution.last_review && (
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1.5 line-clamp-1 italic">
                «{solution.last_review.commentary?.slice(0, 80)}
                {solution.last_review.commentary?.length > 80 ? "…" : ""}»
              </p>
            )}
            ...
            {hasAiScore && (
              <div
                className={`mt-3 rounded-lg border ${aiColor.border} px-3 py-2 flex items-center gap-3`}
              >
                <BsCpu className="w-4 h-4 flex-shrink-0 text-gray-400 dark:text-gray-500" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <span className={`text-xs font-bold ${aiColor.text}`}>
                      {score}%
                    </span>
                    {solution.critical_issues && (
                  ...
                  <div
                    className={`h-1.5 rounded-full ${aiColor.track} overflow-hidden`}
                  >
                    <div
