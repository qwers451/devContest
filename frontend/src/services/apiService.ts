...
const USER_API_URL =
  (import.meta.env.VITE_USER_API_URL as string | undefined) ?? "http://localhost:8001";
const CONTEST_API_URL =
  (import.meta.env.VITE_CONTEST_API_URL as string | undefined) ?? "http://localhost:8002";
const PAYMENT_API_URL =
  (import.meta.env.VITE_PAYMENT_API_URL as string | undefined) ?? "http://localhost:8004";
const EVAL_API_URL =
  (import.meta.env.VITE_EVAL_API_URL as string | undefined) ?? "http://localhost:8003";
...
const IMPORT_EXPORT_API_URL =
  (import.meta.env.VITE_IMPORT_EXPORT_API_URL as string | undefined) ?? CONTEST_API_URL;
...
export const fetchData = async <T = unknown>(
  ...
  try {
    ...
    if (!options.silent) console.error(`Error fetching data from ${endpoint}:`, error);
    throw error;
  }
};
...
export const patchData = async <T = unknown>(
  ...
  try {
    const response = await getClient(endpoint).patch<T>(endpoint, data, { params });
    return response.data;
  } catch (error) {
...
export const downloadFileOrZip = async (endpoint: string, filename: string): Promise<void> => {
  const baseURL = getBaseUrl(endpoint);
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(`${baseURL}${endpoint}`, {
