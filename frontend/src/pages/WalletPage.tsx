...
const WalletPage = () => {
    ...
    const [contestTitles, setContestTitles] = useState<Record<number, string | false>>({});
    ...
    useEffect(() => {
        ...
        const missing = ids.filter(id => contestTitles[id] === undefined && !fetchingTitleIds.current.has(id));
        if (!missing.length) return;
        missing.forEach(id => fetchingTitleIds.current.add(id));
        Promise.all(missing.map(id => contest.fetchOneContestById(id).then(c => [id, c?.title ?? false]).catch(() => [id, false])))
            .then(entries => {
