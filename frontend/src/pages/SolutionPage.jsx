...
const SolutionPage = () => {
    ...
    if (error) {
        return <div className="max-w-4xl mx-auto px-4 py-10 text-red-500">{error}</div>;
    }
    ...
    const handleSelectWinner = async () => {
        try {
            await solution.selectWinner(currentContest.id, currentSolution.id, currentSolution.executor_id);
    ...
    const btnPrimary = 'inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-700 text-white font-semibold text-sm transition-colors';
    const btnSecondary = 'inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-gray-200 bg-white hover:bg-gray-50 text-gray-700 font-semibold text-sm transition-colors';
    const btnDanger = 'inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-red-500 hover:bg-red-600 text-white font-semibold text-sm transition-colors';
    const btnInfo = 'inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-sky-500 hover:bg-sky-600 text-white font-semibold text-sm transition-colors';
    const btnWarning = 'inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-semibold text-sm transition-colors';
    const btnSuccess = 'inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white font-semibold text-sm transition-colors';
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-sm text-gray-500 mb-2">
                                    ...
                                    <span className="font-medium text-violet-600">
                                        @{user.getById(currentContest.customer_id)?.login || 'Неизвестно'}
                                    </span>
                                </p>
                                <span
                                    className="inline-block px-3 py-1 rounded-full text-sm font-semibold"
                                    style={{ color: statusInfo?.textColor, backgroundColor: statusInfo?.color }}
                                >
                                    {statusInfo?.label}
                                </span>
                            </div>
                    ...
                    <div className="px-6 py-4 border-t border-gray-100 bg-gray-50">
                        <div className="flex flex-wrap gap-2 justify-between">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {(isOwner || isAdmin) && (
                                    <>
                                        <button onClick={() => navigate(`/contest/${currentContest.number}`)} className={btnSecondary}>
                                            К конкурсу
                                        </button>
                                        <button onClick={() => navigate('/my-solutions')} className={btnPrimary}>
                                            Мои решения
                                        </button>
                                    </>
                                )}
                                {isEmployer && (
                                    <button onClick={() => navigate(`/contest/${currentContest.number}/solutions`)} className={btnSecondary}>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {(isOwner || isAdmin) && (
                                    <>
                                        <button onClick={() => navigate(`/solution/${currentSolution.number}/reviews`)} className={btnInfo}>
                                            Отзывы
                                        </button>
                                        <button onClick={() => navigate(`/solution/${currentSolution.number}/edit`, { state: JSON.parse(JSON.stringify(currentSolution)) })} className={btnSuccess}>
                                            Редактировать
                                        </button>
                                ...
                                {isEmployer && (
                                    <>
                                        ...
                                        <button onClick={() => setShowStatusModal(true)} className={btnWarning}>
                                            Изменить статус
                                        </button>
                                        <button onClick={() => navigate(`/solution/${currentSolution.number}/create-review`)} className={btnSuccess}>
                                            Оставить отзыв
                                        </button>
                                        {isContestOwner && isContestActive && (
                                            <button onClick={() => setShowWinnerModal(true)} className={btnPrimary}>
                                                🏆 Выбрать победителем
                                            </button>
                                        )}
                                    </>
