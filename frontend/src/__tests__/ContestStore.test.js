...
const CONTEST = {
    id: 1, number: 1, customer_id: 10,
    title: 'Test Contest', annotation: 'Ann',
    description: 'Desc', tz_text: 'TZ',
    prizepool: 5000, status: 'active',
    type_id: 2, files: [], created_at: new Date().toISOString(),
...
describe('Сценарий 7: фильтр по статусу', () => {
    ...
    it('fetchContestsFiltered передаёт status в params', async () => {
        api.fetchData.mockResolvedValue(PAGINATED);
...
describe('Сценарий 8: фильтр по типу', () => {
    it('fetchContestsFiltered передаёт type_id в params', async () => {
        api.fetchData.mockResolvedValue(PAGINATED);
        store.setSelectedTypes([{ id: 2, name: 'Логотип' }]);
        await store.fetchContestsFiltered();
        const params = api.fetchData.mock.calls[0][1];
        expect(params.type_id).toBe(2);
    });
});
...
describe('Сценарий 8b/15: типы конкурсов', () => {
    it('fetchTypes сохраняет список типов', async () => {
        const types = [{ id: 1, name: 'Статья' }, { id: 2, name: 'Логотип' }];
...
describe('Сценарий 19: редактирование этапов', () => {
    it('updateStages вызывает PUT /contests/{id}/stages', async () => {
        ...
        expect(api.updateData).toHaveBeenCalledWith('/contests/1/stages', expect.any(Array));
...
describe('Сценарий 41: статистика', () => {
    it('fetchStatistics вызывает GET /statistics и сохраняет данные', async () => {
        const stats = { x_labels: ['Статья', 'Логотип'], datasets: [{ label: 'Количество', data: [3, 5] }] };
        api.fetchData.mockResolvedValue(stats);
        await store.fetchStatistics('type', 'count');
        expect(api.fetchData).toHaveBeenCalledWith('/statistics', { x: 'type', y: 'count' });
