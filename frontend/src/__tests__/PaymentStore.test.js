...
describe("fetchBalance", () => {
  it("загружает баланс и сохраняет в store", async () => {
    api.paymentApi.get.mockResolvedValue({ data: { balance: 1500 } });
  ...
  it("при ошибке возвращает 0 и не падает", async () => {
    api.paymentApi.get.mockRejectedValue(new Error("Network error"));
    const result = await store.fetchBalance();
...
describe("fetchWalletPaymentStatus", () => {
  ...
  it("при ошибке возвращает null", async () => {
    api.paymentApi.get.mockRejectedValue(new Error("Not found"));
...
describe("topupWallet", () => {
  it("вызывает POST /wallet/topup и возвращает данные", async () => {
    ...
    api.paymentApi.post.mockResolvedValue({ data });
  ...
  it("при status=held вызывает fetchBalance", async () => {
    ...
    api.paymentApi.post.mockResolvedValue({ data });
    api.paymentApi.get.mockResolvedValue({ data: { balance: 200 } });
  ...
  it("при ошибке устанавливает error из detail и бросает исключение", async () => {
    ...
    api.paymentApi.post.mockRejectedValue(err);
  ...
  it("при ошибке без detail использует запасное сообщение", async () => {
    api.paymentApi.post.mockRejectedValue(new Error("Network"));
...
describe("fetchWalletTransactions", () => {
  it("загружает транзакции", async () => {
    ...
    api.paymentApi.get.mockResolvedValue({ data: txs });
  ...
  it("при ошибке устанавливает error", async () => {
    api.paymentApi.get.mockRejectedValue({ response: { data: { detail: "Forbidden" } } });
    await store.fetchWalletTransactions();
  ...
  it("при ошибке без detail использует запасное сообщение", async () => {
    api.paymentApi.get.mockRejectedValue(new Error("Network"));
    await store.fetchWalletTransactions();
...
describe("withdrawFromWallet", () => {
  it("выводит средства и обновляет баланс", async () => {
    ...
    api.paymentApi.post.mockResolvedValue({ data: payout });
  ...
  it("баланс не уходит ниже 0", async () => {
    store.balance = 100;
    api.paymentApi.post.mockResolvedValue({ data: { id: 2 } });
    await store.withdrawFromWallet(500);
    expect(store.balance).toBe(0);
  });

  it("при ошибке устанавливает error и бросает исключение", async () => {
    api.paymentApi.post.mockRejectedValue({ response: { data: { detail: "Not enough" } } });
...
describe("payContestFromBalance", () => {
  it("оплачивает конкурс с баланса и обновляет payment и balance", async () => {
    ...
    api.paymentApi.post.mockResolvedValue({ data: paymentData });
  ...
  it("при ошибке устанавливает error и бросает", async () => {
    api.paymentApi.post.mockRejectedValue({ response: { data: { detail: "Low balance" } } });
...
describe("initPayment", () => {
  ...
  it("при ошибке устанавливает error и бросает", async () => {
    api.paymentApi.post.mockRejectedValue({ response: { data: { detail: "Invalid amount" } } });
...
describe("fetchHistory", () => {
  it("загружает историю платежей", async () => {
    ...
    api.paymentApi.get.mockResolvedValue({ data: list });
...
describe("refundWalletTopup", () => {
  ...
  it("при ошибке устанавливает error и бросает", async () => {
    api.paymentApi.post.mockRejectedValue({ response: { data: { detail: "Already refunded" } } });
...
describe("refundPayment", () => {
  it("возвращает платёж и обновляет history", async () => {
    ...
    api.paymentApi.post.mockResolvedValue({ data: updated });
  ...
  it("при ошибке устанавливает error и бросает", async () => {
    api.paymentApi.post.mockRejectedValue({ response: { data: { detail: "Cannot refund" } } });
...
describe("fetchWithdrawals", () => {
  ...
  it("при ошибке устанавливает error", async () => {
    api.paymentApi.get.mockRejectedValue({ response: { data: { detail: "Not found" } } });
    await store.fetchWithdrawals();
...
describe("activateContest", () => {
  it("активирует конкурс и возвращает true", async () => {
    api.patchData.mockResolvedValue({});
    const result = await store.activateContest(5);
    expect(result).toBe(true);
  });

  it("при ошибке возвращает false", async () => {
    api.patchData.mockRejectedValue(new Error("Forbidden"));
