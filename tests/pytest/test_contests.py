...
async def test_list_contests(customer_token):
    async with httpx.AsyncClient() as c:
        r = await c.get(f"{CONTEST_URL}/contests",
...
async def test_get_contest_by_number(customer_token, contest):
    async with httpx.AsyncClient() as c:
        r = await c.get(f"{CONTEST_URL}/contests/number/{contest['number']}",
                        headers=auth_headers(customer_token))
    assert r.status_code == 200
    data = r.json()
...
async def test_get_contest_not_found(customer_token):
    async with httpx.AsyncClient() as c:
        r = await c.get(f"{CONTEST_URL}/contests/number/999999",
                        headers=auth_headers(customer_token))
...
async def test_edit_contest(customer_token, contest):
    async with httpx.AsyncClient() as c:
        r = await c.put(f"{CONTEST_URL}/contests/{contest['id']}", json={
...
async def test_list_own_contests(customer_token, contest):
    async with httpx.AsyncClient() as c:
        profile = await c.get(f"http://localhost:8001/users/profile",
                              headers=auth_headers(customer_token))
...
async def test_delete_contest_customer_forbidden(customer_token, contest):
    async with httpx.AsyncClient() as c:
        r = await c.delete(f"{CONTEST_URL}/contests/{contest['id']}",
                           headers=auth_headers(customer_token))
    assert r.status_code == 403


@pytest.mark.asyncio
async def test_delete_contest_admin_succeeds(admin_token, contest_type_id, customer_token):
    """Creates a disposable contest and deletes it as admin."""
    async with httpx.AsyncClient() as c:
        cr = await c.post(f"{CONTEST_URL}/contests", json={
        ...
        cid = cr.json()["id"]
        dr = await c.delete(f"{CONTEST_URL}/contests/{cid}",
...
async def test_update_stages(customer_token, contest):
    ...
    async with httpx.AsyncClient() as c:
        r = await c.put(f"{CONTEST_URL}/contests/{contest['id']}/stages",
                        json=new_stages,
                        headers=auth_headers(customer_token))
    assert r.status_code == 200
