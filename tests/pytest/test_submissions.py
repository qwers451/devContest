...
async def test_list_own_submissions(executor_token, submission):
    async with httpx.AsyncClient() as c:
        profile = await c.get("http://localhost:8001/users/profile",
                              headers=auth_headers(executor_token))
        executor_id = profile.json()["id"]
        r = await c.get(f"{CONTEST_URL}/submissions",
...
async def test_get_submission_by_number(customer_token, submission):
    async with httpx.AsyncClient() as c:
        r = await c.get(f"{CONTEST_URL}/submissions/number/{submission['number']}",
                        headers=auth_headers(customer_token))
    assert r.status_code == 200
    data = r.json()
...
async def test_get_submission_not_found(customer_token):
    async with httpx.AsyncClient() as c:
        r = await c.get(f"{CONTEST_URL}/submissions/number/999999",
                        headers=auth_headers(customer_token))
...
async def test_edit_submission(executor_token, submission):
    async with httpx.AsyncClient() as c:
        r = await c.put(f"{CONTEST_URL}/submissions/{submission['id']}", json={
...
async def test_delete_submission_wrong_owner(customer_token, submission):
    async with httpx.AsyncClient() as c:
        r = await c.delete(f"{CONTEST_URL}/submissions/{submission['id']}",
                           headers=auth_headers(customer_token))
    assert r.status_code == 403
...
async def test_delete_submission(executor_token, contest, admin_token):
    ...
    async with httpx.AsyncClient() as c:
        cr = await c.post(f"{CONTEST_URL}/submissions", json={
            "contest_id": contest["id"],
        ...
        sid = cr.json()["id"]
        dr = await c.delete(f"{CONTEST_URL}/submissions/{sid}",
                            headers=auth_headers(executor_token))
    assert dr.status_code == 204
