...
print(
    f"  admin     → {'OK id=' + str(admin_resp['user']['id']) if admin_resp else 'FAIL'}"
)
...
print(
    f"  customer1 → {'OK id=' + str(customer_resp['user']['id']) if customer_resp else 'FAIL'}"
)
...
print(
    f"  executor1 → {'OK id=' + str(executor_resp['user']['id']) if executor_resp else 'FAIL'}"
)
...
print(
    f"  executor2 → {'OK id=' + str(executor2_resp['user']['id']) if executor2_resp else 'FAIL'}"
)
...
if not customer_token:
    ...
    print(
        f"  Логотип для стартапа → {'OK id=' + str(c1['id']) if c1 else f'FAIL {status}'}"
    )
    ...
    print(
        f"  Статья об ИИ       → {'OK id=' + str(c2['id']) if c2 else f'FAIL {status}'}"
    )
    ...
    print(
        f"  Баннеры для рекламы → {'OK id=' + str(c3['id']) if c3 else f'FAIL {status}'}"
    )
